...
const ARG_TOP_RANK_MESSAGES = {
  1: [
    '— ТВОЁ МЕСТО ЗДЭСЬ: НА ВЕРШИНЕ ЦЭПИ ЦИФРОВОЙ СЭТИ.',
    'ТЫ МОЖЭШЬ ПО-ПРАВУ СЧИТАТЬ СЭБЯ ВЫШЭ ДРУГИХ.',
  ].join('\n'),

  2: [
    '— ВТОРОЭ МЭСТО ПО ЦИФРОВОЙ СЭТИ — ТОЖЕ НЭПЛОХО...',
    'МОИ ПОЗДРАВЛЭНИЯ.',
  ].join('\n'),

  3: [
    '— БРОНЗОВЫЙ ВОИН...',
    'СМОЖЭШЬ ЛИ ТЫ ОДОЛЭТЬ ЭЩЁ ДВОИХ ВЫШЭ СЭБЯ ИЛИ ВОВРЭМЯ ОСТАНОВИШЬСЯ?..',
  ].join('\n'),
};
...
app.post('/api/pong/profile/customize', (req, res) => {
  let userId = '';
  try {
    userId = parseInitDataUserIdOrThrow(req);
  } catch {
  ...
  player.avatarSource = avatarSource;
  player.updatedAt = new Date().toISOString();
  writeLeaderboard(players);
  return res.json({ ok: true, player });
});
...
// --------- /emoji_id (временная команда, только для админа) ---------
if (/^\/emoji_id(?:@[\w_]+)?(?:\s|$)/i.test(text)) {
  if (!isAdmin) {
    await sendMessage(fromId, ADMIN_ONLY_REPLY);
    return res.json({ ok: true });
  }
  ...
  if (!replyMessage) {
    await sendMessage(fromId, 'Используй /emoji_id ответом на сообщение с custom emoji.');
    return res.json({ ok: true });
  }
  const customEmojiIds = new Set();
  const collectFromEntities = (entities) => {
    if (!Array.isArray(entities)) return;
    for (const entity of entities) {
      if (entity && entity.type === 'custom_emoji' && entity.custom_emoji_id) {
        customEmojiIds.add(String(entity.custom_emoji_id));
      }
    }
  };
  collectFromEntities(replyMessage.entities);
  collectFromEntities(replyMessage.caption_entities);
  if (replyMessage.sticker?.custom_emoji_id) {
    customEmojiIds.add(String(replyMessage.sticker.custom_emoji_id));
  }
  const ids = Array.from(customEmojiIds);
  if (!ids.length) {
    await sendMessage(fromId, 'custom_emoji_id не найден.');
    return res.json({ ok: true });
  }
  await sendMessage(fromId, `custom_emoji_id:\n${ids.join('\n')}`);
  return res.json({ ok: true });
}
