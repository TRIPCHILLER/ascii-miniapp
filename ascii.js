...
const ARG_PONG = {
  ...
  visorEyeMaxShiftXPx: 31.05,
  visorEyeMaxShiftYPx: 20.7,
  ...
  visorEyePupilParallaxBoost: 1.1,
  visorEyeParallaxMaxXPx: 15.18,
  visorEyeParallaxMaxYPx: 11.27,
  ...
  visorRoundShakeSpeedFactor: 0.8,
  visorRoundPupilShakeRatio: 0.42,
  ...
  visorEyeCountdownFastShakeAmpYPx: 20,
  visorEyeCountdownFastShakeSpeedY: Math.PI * 2 / 46,
  visorPupilCountdownFastShakeAmpYPx: 16.4,
  visorPupilCountdownFastShakeSpeedY: Math.PI * 2 / 44,
...
const ARG_RESULT_REPLIES = {
  ...
  function safePushArgDebug(event, data = {}) {
    try {
      pushArgDebug(event, data);
    } catch (err) {
      console.warn('[arg-debug-failed]', event, err?.message || err);
    }
  }
  ...
  async function finishArgMatch(snapshot = null) {
    ...
    safePushArgDebug('finishArgMatch-enter', {
    ...
    try {
      ...
      safePushArgDebug('finishArgMatch-before-api', {
        reason: finishSnapshot.reason,
      ...
      safePushArgDebug('finishArgMatch-after-api', {
        reason: finishSnapshot.reason,
        playerScore: runScore,
        aiScore: botScore,
        hasRunId: Boolean(finishSnapshot.runId),
      ...
      safePushArgDebug('finishArgMatch-before-result-ui', {
        reason: finishSnapshot.reason,
        playerScore: runScore,
        aiScore: botScore
      });
...
ЛИЧНЫЙ Р3К0РД: [${resultMeta.bestScore}]`, {
      ...
      safePushArgDebug('finishArgMatch-done', {
        reason: finishSnapshot.reason,
        playerScore: runScore,
        aiScore: botScore
      });
      ...
      safePushArgDebug('finishArgMatch-error', {
        reason: finishSnapshot.reason,
  ...
  function resetArgBall(serveToPlayer = true) {
    ...
    argPongState.ballTargetScale = 1;
  ...
  function startArgBossIntroLoop({ overlay, visorBody, visorEye, visorPupil }) {
    ...
    const loop = (ts) => {
      ...
      argPongState.visorVX += (0 - argPongState.visorShiftX) * ARG_PONG.visorEyeSpring;
      argPongState.visorVY += (0 - argPongState.visorShiftY) * ARG_PONG.visorEyeSpring;
      ...
      const visorEyeTargetX = clamp(
        argPongState.visorShiftX * ARG_PONG.visorEyeParallaxFollow * ARG_PONG.visorEyePupilParallaxBoost,
        -ARG_PONG.visorEyeParallaxMaxXPx * ARG_PONG.visorFollowRadiusBoost * ARG_PONG.visorEyePupilParallaxBoost,
        ARG_PONG.visorEyeParallaxMaxXPx * ARG_PONG.visorFollowRadiusBoost * ARG_PONG.visorEyePupilParallaxBoost
      );
      const visorEyeTargetY = clamp(
        argPongState.visorShiftY * ARG_PONG.visorEyeParallaxFollow * ARG_PONG.visorEyePupilParallaxBoost,
        -ARG_PONG.visorEyeParallaxMaxYPx * ARG_PONG.visorFollowRadiusBoost * ARG_PONG.visorEyePupilParallaxBoost,
        ARG_PONG.visorEyeParallaxMaxYPx * ARG_PONG.visorFollowRadiusBoost * ARG_PONG.visorEyePupilParallaxBoost
      );
  ...
  function startArgPong({ overlay, ballStickLayer, ball, topStick, bottomStick, visorBody, visorEye, visorPupil, scoreLayer, aiScoreEl, playerScoreEl, preserveBossState = false }) {
    ...
    const triggerArgMatchFinish = (reason) => {
      safePushArgDebug('finish-trigger-enter', {
      ...
      safePushArgDebug('finish-trigger-before-finishArgMatch', {
        reason: finishSnapshot.reason,
        playerScore: finishSnapshot.playerScore,
        aiScore: finishSnapshot.aiScore,
        hasRunId: Boolean(finishSnapshot.runId)
      ...
      setTimeout(() => {
        if (argMatchFinishing && !argMatchFinishDone) {
          safePushArgDebug('finish-watchdog-timeout', {
            aiScore: argPongState?.aiScore,
            playerScore: argPongState?.playerScore,
            hasRunId: Boolean(argPongState?.runId)
          });
    ...
    const loop = () => {
      ...
      if (!serveLocked && argPongState.ballVY < 0 && argPongState.ballY < topGoalLine) {
        ...
        safePushArgDebug('ai-score-increment', {
          aiScore: argPongState.aiScore,
          playerScore: argPongState.playerScore,
          hasRunId: Boolean(argPongState.runId)
        });
      ...
      const eyePupilParallaxBoost = ARG_PONG.visorEyePupilParallaxBoost;
      const visorEyeTargetX = clamp(
        clampedTargetX * ARG_PONG.visorEyeParallaxFollow * clutchParallaxBoost * eyePupilParallaxBoost,
        -ARG_PONG.visorEyeParallaxMaxXPx * ARG_PONG.visorFollowRadiusBoost * clutchParallaxBoost * eyePupilParallaxBoost,
        ARG_PONG.visorEyeParallaxMaxXPx * ARG_PONG.visorFollowRadiusBoost * clutchParallaxBoost * eyePupilParallaxBoost
      );
      const visorEyeTargetY = clamp(
        clampedTargetY * ARG_PONG.visorEyeParallaxFollow * clutchParallaxBoost * eyePupilParallaxBoost,
        -ARG_PONG.visorEyeParallaxMaxYPx * ARG_PONG.visorFollowRadiusBoost * clutchParallaxBoost * eyePupilParallaxBoost,
        ARG_PONG.visorEyeParallaxMaxYPx * ARG_PONG.visorFollowRadiusBoost * clutchParallaxBoost * eyePupilParallaxBoost
      );
      argPongState.visorEyeShiftX += (visorEyeTargetX - argPongState.visorEyeShiftX) * 0.22;
      argPongState.visorEyeShiftY += (visorEyeTargetY - argPongState.visorEyeShiftY) * 0.22;
      ...
      const visorPupilTargetOffsetX = argPongState.visorShiftX * 1.25 * eyePupilParallaxBoost + argPongState.visorPupilRecoilX;
      const visorPupilTargetOffsetY = argPongState.visorShiftY * 1.25 * eyePupilParallaxBoost + argPongState.visorPupilRecoilY;
      ...
      const visorPupilOffsetY = argPongState.visorPupilShiftY + roundSpringShakeY * ARG_PONG.visorRoundPupilShakeRatio;
  ...
  function bindModeChooserOnce() {
    ...
    const renderStartEyeAscii = (motion = {}) => {
      ...
      if (!START_EASTER_EYE_DEBUG_HIDE_PUPIL) {
        ...
        const pupilDrawW = pupilAssetW * pupilUniformScale * START_EASTER_EYE_PUPIL_WIDTH_BOOST;
    ...
    const startStartMenuBigEyeShakeForAudio = (audio, soundIndex) => {
      ...
      const loop = (ts) => {
        ...
        const shakeAccel = 0.16 + 0.84 * Math.pow(shakeProgress, 1.2);
    ...
    const startEasterRoulette = (soundIndex, { durationMs = 4200, onComplete = null } = {}) => {
      ...
      const tick = () => {
        ...
        const minDelay = 20;
        const maxDelay = 460;
        const easedPhase = 0.18 + 0.82 * Math.pow(phase, 1.05);
...
if (name.includes('notallowed'))
  ...
  if (name.includes('notfound') || name.includes('overconstrained'))
    return { 
      ...
      extra: 'Зачем же ты тогда пришёл ко мне?..'
    };
  ...
  if (name.includes('security'))
    return { 
      ...
      message: 'Запусти меня в защищёщнном контексте HTTPS и проверь права.'
    };
