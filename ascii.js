...
window.onerror = function (m, s, l, c, e) {
  alert('JS ERROR: ' + (e?.message || m));
};
window.addEventListener('unhandledrejection', (ev)=>{
  alert('PROMISE ERROR: ' + (ev.reason?.message || ev.reason));
});

// ===== mini-popup =====
function debug(msg){ alert(String(msg)); }

// ===== ВРЕМЕННО: тестовый blob, чтобы проверить сам POST =====
async function exportCurrentResultBlob() {
  // Если твой экспорт работает — можешь закомментить строку ниже.
  return new Blob(['TEST_ASCII_VISOR'], { type: 'text/plain' });
  // --- Ниже твой реальный экспорт (вернёшь когда сеть подтвердим) ---
...
async function onSaveClick(e){
  try {
    ...
    const blob = await exportCurrentResultBlob();
    if (!blob) return debug('3) ERR: blob empty');
    debug('3) blob size=' + (blob.size||0));

    const form = new FormData();
    form.append('file', blob, 'ascii.txt'); // ВАЖНО: ключ "file"

    const url = 'https://api.tripchiller.com/api/upload';
    const headers = {};
    if (isTg) headers['x-telegram-init-data'] = Telegram.WebApp.initData || '';

    debug('4) before fetch → ' + url);
    const res = await fetch(url, { method:'POST', body: form, headers });
    const text = await res.text();
    debug('5) status=' + res.status + ' body=' + text.slice(0,200));
  } catch (e) {
    debug('ERR fetch: ' + (e?.message||e));
  }
}

// ===== Навешиваем слушатель на кнопку (и говорим, нашли ли её) =====
(function attachSaveHandler(){
  const btn = document.querySelector('#saveBtn, [data-action="save"], button.save, .save-btn');
  if (!btn) { alert('Save button not found'); return; }
  alert('save handler attached');
  btn.addEventListener('click', onSaveClick);
})();
